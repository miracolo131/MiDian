    ...
    if (!result) {
		...
		if ([status isEqualToString:@"OK"]) {
			...
			if ([status isEqualToString:@"ERROR"]) {
                ...
                int errorCode = [errorDic[@"errorCode"] intValue];
                NSString *errorMessage = errorDic[@"errorMessage"];
                NSError *error = [NSError errorWithDomain:errorMessage code:errorCode userInfo:errorDic];
                [self failedWithError:error];
                NSLog(@"%@", errorMessage);
			}
		}
